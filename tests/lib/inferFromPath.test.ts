...
describe('inferFromPath', () => {
  ...
  describe('legacy paths', () => {
    ...
    it('archetype-features/{id}.md → archetype-feature, id, no archetypeId', () => {
      expect(inferFromPath('archetype-features/abducted.md')).toEqual({
        type: 'archetype-feature', id: 'abducted',
      });
    });
    it('class-features/{cid}/{id}.md → class-feature, className, id', () => {
      expect(inferFromPath('class-features/shifter/shifter-casting.md')).toEqual({
        type: 'class-feature', className: 'shifter', id: 'shifter-casting',
      });
    });
    it('class-traits/{cid}/{id}.md → class-trait, className, id, no featureId', () => {
      expect(inferFromPath('class-traits/armorist/armorist-additional-binding.md')).toEqual({
        type: 'class-trait', className: 'armorist', id: 'armorist-additional-binding',
      });
    });
    it('articles/{id}.md → article, id', () => {
      expect(inferFromPath('articles/using-spheres.md')).toEqual({
        type: 'article', id: 'using-spheres',
      });
    });
  ...
  describe('new sphere-nested paths', () => {
    ...
    it('spheres/{sid}/talents/{id}.md → talent, sphere, id', () => {
      expect(inferFromPath('spheres/alteration/talents/multifarious-transformation.md')).toEqual({
        type: 'talent', sphere: 'alteration', id: 'multifarious-transformation',
      });
    });
    it('spheres/{sid}/feats/{id}.md → feat, sphere, id', () => {
      expect(inferFromPath('spheres/alteration/feats/toxic-webs.md')).toEqual({
        type: 'feat', sphere: 'alteration', id: 'toxic-webs',
      });
    });
  });
  ...
  describe('new class-nested paths', () => {
    ...
    it('classes/{cid}/features/{id}.md → class-feature, className, id', () => {
      expect(inferFromPath('classes/shifter/features/shifter-casting.md')).toEqual({
        type: 'class-feature', className: 'shifter', id: 'shifter-casting',
      });
    });
    it('classes/{cid}/features/{fid}/traits/{id}.md → class-trait, className, featureId, id', () => {
      expect(inferFromPath('classes/armorist/features/arsenal-trick/traits/armorist-additional-binding.md')).toEqual({
        type: 'class-trait', className: 'armorist', featureId: 'arsenal-trick', id: 'armorist-additional-binding',
      });
    });
    it('classes/{cid}/archetypes/{aid}/index.md → archetype, className, id=aid', () => {
      expect(inferFromPath('classes/cleric/archetypes/abductee/index.md')).toEqual({
        type: 'archetype', className: 'cleric', id: 'abductee',
      });
    });
    it('classes/{cid}/archetypes/{aid}/features/{id}.md → archetype-feature, archetypeId, id', () => {
      expect(inferFromPath('classes/cleric/archetypes/abductee/features/abducted.md')).toEqual({
        type: 'archetype-feature', archetypeId: 'abductee', id: 'abducted',
      });
    });
  });
