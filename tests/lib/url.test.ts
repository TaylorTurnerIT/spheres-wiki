...
vi.mock("../../src/lib/url", async () => {
  const actual =
    await vi.importActual<typeof import("../../src/lib/url")>(
      "../../src/lib/url",
    );
  return actual;
});
...
describe("url", () => {
  ...
  it("strips trailing slash from base URL before concatenating", () => {
    ...
    expect(`${"/spheres-wiki/".slice(0, -1)}/about/`).toBe(
      "/spheres-wiki/about/",
    );
  });
  ...
  it("preserves leading slash on path", () => {
    expect(`${"/spheres-wiki/".slice(0, -1)}/search/`).toBe(
      "/spheres-wiki/search/",
    );
  });
  ...
  it("handles path without trailing slash", () => {
    expect(`${"/spheres-wiki/".slice(0, -1)}/power`).toBe(
      "/spheres-wiki/power",
    );
  });
});
