...
import { SYSTEMS, SYSTEM_DEFAULT, HEADER_NAV, SITE_TITLE, SITE_TAGLINE, ANNOUNCEMENT } from '../../src/config/site';
...
describe('site.ts — SYSTEMS record', () => {
  ...
  it('each system has required fields', () => {
    const requiredFields: Array<keyof SystemConfig> = [
      'label', 'color', 'darkColor', 'route', 'cssKey',
      'subtitle', 'classLabel', 'description', 'introLinkText',
      'buildText', 'buildHref',
    ];
    for (const id of systemIds) {
  ...
  it('all routes start and end with /', () => {
    for (const id of systemIds) {
      ...
      expect(route.startsWith('/'), `${id} route does not start with /`).toBe(true);
  ...
  it('dark colors are darker than main colors (value comparison)', () => {
    for (const id of systemIds) {
      ...
      expect(darkVal, `${id} darkColor should be numerically lower than color`).toBeLessThan(mainVal);
    }
  });
...
describe('site.ts — core constants', () => {
  ...
  it('ANNOUNCEMENT is either a string or null', () => {
    expect(typeof ANNOUNCEMENT === 'string' || ANNOUNCEMENT === null).toBe(true);
  });
});
