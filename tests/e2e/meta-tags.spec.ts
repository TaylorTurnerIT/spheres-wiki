...
test.describe("Meta tags", () => {
  ...
  test("security headers are present", async ({ page }) => {
    ...
    await expect(referrer).toHaveAttribute(
      "content",
  ...
  test("detailed pages have non-generic descriptions (T27)", async ({
    page,
  }) => {
    ...
    // T27 not yet done — when implemented, descriptions will be page-specific.
    // For now, accept the generic fallback.
    if (content === "A quick reference site for the Spheres system.") {
      console.warn(
        "T27: per-page descriptions not yet implemented — using generic fallback.",
      );
    }
  });
});
