...
test.describe("Navigation — sidebar", () => {
  // Sidebar toggle (hamburger) is only visible on mobile viewports
  ...
  test("sidebar links do not 404", async ({ page }) => {
    ...
    const links = await page
      .locator("#site-sidebar a[href]")
      .evaluateAll((els) =>
        els.map((el) => (el as HTMLAnchorElement).getAttribute("href")!),
      );
    // Filter to internal links only
    const internalLinks = links.filter(
      (href) => href.startsWith("/") || href.startsWith("http://localhost"),
    );
    for (const href of internalLinks) {
      const response = await page.request.get(href);
      expect(
        response.status(),
        `Sidebar link ${href} should not 404`,
      ).toBeLessThan(400);
    }
  });
});
...
test.describe("Navigation — header nav", () => {
  test("header nav links do not 404", async ({ page }) => {
    ...
    for (const href of links) {
      ...
      expect(
        response.status(),
        `Header link ${href} should not 404`,
      ).toBeLessThan(400);
    }
  });
});
...
test.describe("Navigation — View Transitions (V11/B1)", () => {
  test("search bar is initialized after client-side navigation", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("sidebar toggle works after client-side navigation", async ({
    page,
  }) => {
...
test.describe("Navigation — TabNav", () => {
  ...
  test("TabNav links do not 404", async ({ page }) => {
    ...
    for (const href of links) {
      ...
      expect(
        response.status(),
        `TabNav link ${href} should not 404`,
      ).toBeLessThan(400);
    }
  });
});
