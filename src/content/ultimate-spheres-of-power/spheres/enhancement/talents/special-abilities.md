...
__*Special Materials (Ex; 1+ CP)*__
...
**Slashing Attack (Ex, 1 CP):** Replace one slam attack with an attack that does slashing damage and has either a 19–20 threat range (for blade-like attacks) or a ×3 threat multiplier (for axe or scythe-like attacks). Replace all melee attacks for an additional +1 CP. Animated object abilities that specify slam attacks do not work on slashing attacks.
...
**Emulated Creature (Ex; +2 CP) [3PP]:** The animated object is a living creature designed to mimic an existing phenotype; change its type to either animal, humanoid, outsider (native), or vermin, give it a Constitution score equal to its Strength score, remove construct traits (Ex), and replace its hardness with an equal amount of damage reduction bypassed by magic. The animated object gains a bonus to Disguise checks made to emulate a specific type of creature (such as human or cat) equal to your caster level. It is still mindless.
