...
If an illusion would have a negated effect if disbelieved, a shadow version of that illusion instead has its effectiveness reduced by half (rounded down). For saving throws, this changes the saves to Fort (partial) or Will (partial) instead of Fort (negates) or Will (negates) as applicable. Creatures with the Stalwart ability (or similar) features may choose to treat shadow illusions as normal illusions upon successfully disbelieving the illusion.
