...
After creating a poison, you may make a melee touch attack against a target (if using a contact poison) or throw the poison (if using an inhaled poison) as a standard action. Thrown poisons have a 10 foot range increment. A poison remains potent until used or for 1 round +1 per 4 ranks in Craft (alchemy) you possess, whichever comes first. Inhaled poisons are considered splash weapons that fill up a 10 ft. cube when used.
