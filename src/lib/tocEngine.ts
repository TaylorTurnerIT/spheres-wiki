...
const PRIORITY_CLASSES = [
  ...
  'section-heading-text', 'page-title', 'class-traits-section', 'equipment-section',
];
...
export interface TocEngineOptions {
  ...
  onChange?: (active: HTMLElement | null, activeSub: HTMLAnchorElement | null) => void;
}
...
function defaultApplyActive(categories: HTMLElement[], active: HTMLElement | null) {
...
export function createTocEngine(opts: TocEngineOptions): TocEngine | null {
  ...
  function recalc() {
    ...
    for (const link of allSubLinks) link.classList.toggle('is-current', link === activeSub);
