...
**Feature integration — "built but not wired" guard (V65–V69)**

- V65. New content-collection entry type ! reachable: ∀ new type resolved into `ResolvedMaps` → rendered on ≥1 route ∧ present in search manifest (`search/index.astro`) ∧ Pagefind-indexed. ⊥ orphaned collection (entries on disk + in maps but rendered nowhere). Intentional defer ! have tracked §T row + documented Open Decision in the governing kit; silent invisibility forbidden.
- V66. Logic/lib module ! consumed by ≥1 production page/component, not only by `tests/`. Pure-logic lib whose sole importer is a test = incomplete feature, ⊥ counted done. Run `fallow dead-code --trace <file>:<export>` when the only caller is a test.
- V67. Validation/calculation logic with a preset/loaded-entry path ! covered by ≥1 test that drives it from real resolved content (actual `ResolvedMaps` entries), not only synthetic inline fixtures. Fixture-only coverage ≠ proof the production data path works.
- V68. Entry rule-model fields (e.g. `tradition.cam`, drawback/boon/tradition `rules`) ! honored by the logic that validates/derives from selections of that entry. Schema field with no reader = inert decoration; ⊥ mark feature done while declared rule fields go unread.
- V69. Feature §T task naming a component/page/UI ⊥ marked `x` until its user-facing surface exists ∧ SPEC §T status reconciled. "Logic passes unit tests" is status `~` (partial), not `x`. On feature completion, update the governing kit's status and SPEC §T in the same change.

**Entry rendering — named-entry card primitive (V70–V71)**
...
| B18 | 2026-06-18 | Casting-traditions builder logic (`src/lib/castingTraditions/`) built + tested but imported only by `tests/`; no page renders it, no Builder tab. 364 structured `drawback`/`boon`/`tradition` entries resolve into `ResolvedMaps` but render nowhere and are absent from the search manifest — whole data+logic layer reaches zero users | V65,V66,V69 / T107,T108 |
| B19 | 2026-06-18 | `getAllowedCastingAbilities` (`rules.ts`) reads only boon `rules`; ignores `tradition.cam`, tradition `rules`, drawback `rules`. Preset traditions validate wrong CAM — Blood Magic (`cam fixed [con]`) wrongly permits int/wis/cha; Demonology (`highest [cha,con]`) wrongly permits int/wis | V68 / T109 |
| B20 | 2026-06-18 | No `selectionFromTradition` hydrator: `buildTraditionState({traditionId})` loads tradition `choices`/`choiceSelections` but not its `drawbacks`/`boons` arrays; a preset tradition cannot be validated/loaded without manually re-passing every ref | V67 / T109 |
