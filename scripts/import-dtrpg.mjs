...
if (args.length < 2) {
  console.error(
    "Usage: node import-dtrpg.mjs <book-slug> <curl_command_file.txt>",
  );
  process.exit(1);
}
...
if (!urlMatch) {
  console.error(
    "Could not parse URL from curl command. Make sure it's 'Copy as cURL' format.",
  );
  process.exit(1);
}
...
try {
  ...
  const imgMatch = html.match(
    /<meta property="og:image"[\s\S]*?content="([^"]+)"/i,
  );
  const priceMatch = html.match(
    /<meta itemprop="price"[\s\S]*?content="\$?([^"]+)"/i,
  );
  ...
  if (imageUrl) {
    ...
    const imgPath = path.join(
      __dirname,
      `../src/assets/covers/${coverImageName}`,
    );
