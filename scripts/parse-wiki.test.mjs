...
const bodyCtx   = { type: 'talent', tier: 'basic',    sectionTags: ['body'] };
...
describe('cleanBody', () => {
  ...
  it('converts **Special:** mid-sentence to *Special:*', () => {
    const out = cleanBody('You gain a swim speed. **Special:** When you do this...');
    assert.ok(out.includes('*Special:*'));
    assert.ok(!out.includes('**Special:**'));
  });
...
describe('parseSectionContext', () => {
  ...
  it('"Advanced Blood Talents" → advanced talent', () => {
    ...
    assert.deepEqual(ctx, { type: 'talent', tier: 'advanced', sectionTags: [] });
  });

  it('"Body Talents" → basic talent with body sectionTag', () => {
    const ctx = parseSectionContext('Body Talents');
    assert.deepEqual(ctx, { type: 'talent', tier: 'basic', sectionTags: ['body'] });
  });
  ...
  it('"Transformation Talents" → basic talent with transformation sectionTag', () => {
    ...
    assert.deepEqual(ctx, { type: 'talent', tier: 'basic', sectionTags: ['transformation'] });
  });
...
describe('parseHeading', () => {
  ...
  it('parses name with BRACKET_TAG and source key', () => {
    const result = parseHeading('++++ Name [instill] [BaP]', basicCtx, testConfig);
  ...
  it('parses multiple bracket ability-type tags', () => {
    const result = parseHeading('++++ Name [mass] [range]', basicCtx, testConfig);
  ...
  it('(quicken, still) paren → tags: quicken and still', () => {
    const result = parseHeading('++++ Name (quicken, still)', basicCtx, testConfig);
    assert.ok(result.tags.includes('quicken'));
    assert.ok(result.tags.includes('still'));
  });

  it('(blood art, still) [BaP] → tags: blood-art, still; sourceKey: BaP', () => {
    const result = parseHeading('++++ Name (blood art, still) [BaP]', basicCtx, testConfig);
  ...
  it("normalizes curly apostrophe in name: Puppet’s Curse → Puppet's Curse", () => {
    const result = parseHeading("++++ Puppet’s Curse [BaP]", basicCtx, testConfig);
...
describe('resolveSourceBook', () => {
  ...
  it('null sourceKey → returns primaryBook', () => {
    assert.equal(resolveSourceBook(null, null, config), 'spheres-of-power-core');
  });
  ...
  it('null-mapped Apoc + non-matching bodySource → unknown-source', () => {
    assert.equal(resolveSourceBook('Apoc', 'Completely Different Book', config), 'unknown-source');
  });
  ...
  it('curly apostrophe in bodySource matches straight apostrophe key in bodySourceMap', () => {
    ...
    assert.equal(resolveSourceBook('Apoc', 'Baron’s Arena', cfg), 'barons-arena');
  });
});
...
describe('parseEntryBlock', () => {
  it('returns null when no ++++ heading found', () => {
    const result = parseEntryBlock('Just some text\nno heading', basicCtx, testConfig);
