...
async function migrateBook(bookSlug) {
  ...
  for (const file of classFiles) {
    ...
    const newFrontmatterStr = rebuildFrontmatter(rawFrontmatter, ['type', 'system']);
  ...
  try {
    ...
    for (const file of featureFiles) {
      ...
      const content = await fs.readFile(path.join(classFeaturesDir, file), 'utf8');
      const { frontmatter, body, rawFrontmatter } = parseFrontmatter(content);
      
      const parentClassId = (frontmatter.className || '').toLowerCase().replace(/[\s_]+/g, '-');
      
      if (!parentClassId) {
        console.warn(`  Warning: Class Feature ${file} has no className. Leaving in flat dir.`);
        continue;
      }
      
      const targetFeatureDir = path.join(newClassesDir, parentClassId, 'Class Features');
      await fs.mkdir(targetFeatureDir, { recursive: true });
      
      // Remove type, system, className
      const newFrontmatterStr = rebuildFrontmatter(rawFrontmatter, ['type', 'system', 'className']);
      const newContent = `${newFrontmatterStr}${body}`;
  ...
  try {
    ...
    for (const file of archetypes) {
      ...
      if (!parentClassId) {
        console.warn(`  Warning: Archetype ${file} has no className. Leaving in flat dir.`);
        continue;
      }

      archetypeToClassMap[archetypeId.toLowerCase().replace(/[\s_]+/g, '-')] = parentClassId;
      
      const targetArchDir = path.join(newClassesDir, parentClassId, 'Archetypes', archetypeId);
  ...
  try {
    ...
    for (const file of archFeatureFiles) {
      ...
      const content = await fs.readFile(path.join(archetypeFeaturesDir, file), 'utf8');
      const { frontmatter, body, rawFrontmatter } = parseFrontmatter(content);
      
      const parentArchetypeId = (frontmatter.archetype || frontmatter.archetypeId || '').toLowerCase().replace(/[\s_]+/g, '-');
      const parentClassId = (frontmatter.className || archetypeToClassMap[parentArchetypeId] || '').toLowerCase().replace(/[\s_]+/g, '-');
      
      if (!parentClassId || !parentArchetypeId) {
        console.warn(`  Warning: Archetype Feature ${file} missing className or archetype. Leaving in flat dir.`);
        continue;
      }
      
      const targetArchFeatureDir = path.join(newClassesDir, parentClassId, 'Archetypes', parentArchetypeId, 'Archetype Features');
      await fs.mkdir(targetArchFeatureDir, { recursive: true });
      
      // Remove type, system, className, archetype, archetypeId
      const newFrontmatterStr = rebuildFrontmatter(rawFrontmatter, ['type', 'system', 'className', 'archetype', 'archetypeId']);
      const newContent = `${newFrontmatterStr}${body}`;
