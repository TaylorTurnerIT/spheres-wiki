...
const OUT_DIR = join(ROOT, 'src/content/ultimate-spheres-of-power/class-traits/shifter');

const MODE = process.argv[2] ?? '--dry-run';

// ─── Source mappings ─────────────────────────────────────────────────────────

const LOWERCASE_WORDS = new Set(['a','an','the','and','or','of','in','on','at','to','for','nor','but','yet','so','with','by','from','into','up','as']);
...
if (sectionStart === -1 || sectionEnd === -1) {
  console.error('Could not locate Bestial Trait section boundaries in shifter-class.md');
  process.exit(1);
}
...
function parseChunk(chunk) {
  ...
  const reqLineIdx = bodyLines.findIndex(l => /^\*\*Requires:\*\*/i.test(l.trim()));
...
function renderFile(trait) {
  ...
  const sourceLine = trait.sourceKey
    ? `\n\n*Source: ${SOURCE_MAP[trait.sourceKey] ?? trait.sourceKey}*`
    : '';
